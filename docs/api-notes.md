...
- 鉴权来源：PC 官方页面以 `pnid` Cookie 作为登录存在标记；有效性仍以接口响应为准，Cookie 值不记录。
- 登录状态 API：`GET https://www.piaoniu.com/api/v1/user`；匿名真实请求返回 HTTP 401，已验证。已登录响应待 Edge 会话验证。
- 购票人列表/创建 API：待验证。
- 订单预览 API：待验证。
...
- 登录状态 API：`GET /userapi/user/000/info`；匿名真实请求返回 HTTP 200、业务码 `statusCode=1005`，已验证。已登录响应待 Edge 会话验证。
- 购票人列表/创建 API：待验证。
- 订单预览 API：待验证。
- 创建订单 API：待验证；尚未产生待支付订单。
- 订单详情/列表 API：待验证。
