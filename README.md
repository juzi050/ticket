...
- 票牛适配器可以打开官方首页登录入口，并使用可配置的可见元素检查登录状态；
- 摩天轮移动站未能从公开页面可靠确认登录入口和登录态元素，默认只打开官方首页，必须补充已验证的选择器；
- 两个平台的票务列表、订单确认和锁库存页面没有可靠的公开接口或稳定页面结构，因此相关方法会明确抛出“待适配”，不会猜测接口、伪造选择器或绕过平台限制；
- Mock 模式完整可用，是当前验证系统整体流程的标准方式。
...
platforms:
  motianlun:
    ...
    login_url: https://经过人工核验的官方登录页
    auth_check_url: https://经过人工核验的个人中心页
...
不要填写猜测的 URL 或选择器。`authenticated_selectors` 为空或没有命中时，系统会保守判定为未登录。
...
要继续实现票牛或摩天轮，请提供以下脱敏资料，且确保来源和用途符合平台规则：

1. 未登录、已登录、详情页、选场次、选票档、订单确认页的 Playwright 页面快照或 HTML 结构；
2. 人工操作后的稳定 URL 变化和可见元素；
3. 页面展示的价格、服务费、配送费、库存、连座和最终应付金额所在元素；
4. 正常人工购票路径中各步骤的稳定选择器；
5. 验证码、风控和人工确认提示的可见元素。

在 `app/platforms/piaoniu.py` 或 `app/platforms/motianlun.py` 中实现 `query_tickets` 和 `lock_order`。`lock_order` 必须先读取订单确认页最终金额，再决定是否点击提交；遇到任何验证应返回人工处理状态，并停留在当前页面。禁止接入未经授权的内部接口。
...
项目刻意不虚构未知接口和页面选择器。需要按上一节提供当前页面资料后完成适配。
